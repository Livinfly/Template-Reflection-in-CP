...
-   c，优先队列，模拟去做即可。或者考虑到最多转一圈，那么选定一个开头只要长度 2N 一直取 min 也可以 O(n) 解决。

-   d，RT-Max（重构树-最大），重构之后，边权节点作为 lca 的次数 * 价值，即其左子树大小乘右子树大小。

-   e，贪心。钦定从左往右放，当前要放的位置，p 从 1 开始，如果有左端点等于 p ， 放进优先队列作为可选，把所有可选区间确定之后，若当前位置的可选区间为空，则直接跳到下一个区间左端点最小的左端点位置，否则查看当前要用当前 p 的位置消掉的区间是否满足在区间右端点左边，不满足则非法。 #感觉很典但是又不会的题，有空再看看（

-   f，DP。首先要注意它的典题，从后往前枚举倒数第二位，若和前一个一样，把它的方案拿过来就可以结束了。

    遇到需要用到负数数组的时候，可以整体偏移。

    -   同时有提到**字符串中有多少个不同子序列**的典题。

## abc215

-   d，【调和级数复杂度】【筛法】。 gcd 等于 0 和质因数分解的关系。写得质因数分解还是需要 i <= x/i 的，不然最后放进去的 x 可能是个合数。
-   e，【状压dp】。状态为，考虑前 i 位，选过的比赛类型，最后选的类型。典的状压dp了吧，但是不太会。#重做
-   f，【二分答案】【单调队列】，用单调队列保持“非法区域”，用右端点不断和左边已经被踢出“非法区域”的 y 比较。#重做

## abc216

-   d，自己用 m 个队列，和 set 写了个递归实现，感觉写得不是很流畅，并且丑陋，复杂度应该是为 $O(nlogn)$。#重做

    用vector存下数量，同时存下是具体的编号，就不用另外用别的东西增删列表。$O(n)$

-   e，【二分答案】。显然尽量选大的删，二分最终删到的数值，多的再另外减掉就行。

-   f，【01背包记方案数】。按 $<A_i, B_i>$ 排序，题目是记方案数，可以等价变为，前 i-1 个物品，代价不超过 $A_i - B_i$ 的方案数，就可以背包来做了。其中，排序是为了，可以在前 i-1 个物品里随便选，保持 Max 不变。

## abc217

-   c， q[p[i]] = i 。

-   d，【离线】【DSU】。做法还是很神奇的，把切开变为合并， 就好用dsu做了。记得离散化。#重做

-   e，【优先队列】。用优先队列一直维持有序的，弹出的必然是有序中的最小的，那就只需要一个队列和一个优先队列即可。

    感觉很典，但我不太熟悉。#重做

-   f，【区间DP】。区间dp有点忘了，是要枚举 k 的，因为有区间和区间的合并。最后乘组合数是由于记录方案组合的顺序不同，视为不同方案，确定一边顺序。

    dp[l, r] = dp[l, k-1] * dp[k+1, r] * C(len/2, (r-k+1)/2)

    有点神仙的，现在还悟不到什么东西。。#重做

## abc218

-   c，感觉这种统一偏移的还是很通用的方法。#重做

-   d，枚举矩形斜对角的点，然后用 set 之类的判断另外斜对角的点是否存在。

-   e，【最小生成树MST】。首先满足联通，然后大于 0 的边权加入答案。

-   f，显然，没限制的最短路最多只经过 n-1 条边，对于不是无限制最短路上的边的最短路，就是原最短路；不然就重跑最短路，最多跑 n-1 次。$O((n+m) \cdot n)$

    很典，不会！#重做

## abc219

-   c，单纯提下写法，用 map 将原字符串映射到新大小关系的字符串中，另开 order 数组，记录映射后排序的顺序，再以这个顺序输出原数组。jiangly 貌似是用两个数组进行映射过去，排序完再映射回来。
-   d，【二维DP】。挺裸的。
-   e，【状态压缩】。解法不难想，但没细想。枚举2^16个状态想到了，但是没继续想如何一个一个满足条件。如此枚举，3~5满足，需要再判是否只有一个连通块，是否不是两条河。还是挺有意思的。#重做
-   f，

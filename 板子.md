...
看了 CCSP 2021 B ，感觉先只维护要求的答案的期望，然后慢慢把求期望要用的参数，放入期望维护（用结构体或者另外开数组，维护平方就用 $E((x+1)^2)=E(x^2)+2E(x)+1$ 。

分成两段，中间要修改的话，因为转移是一样的，可能矩阵一下。
...
无根树的树哈希，只要找重心再做有根树的树哈希比较就行（两个重心就能否依次对得上）。
...
ULL shift(ULL x) {
    x ^= mask; // 或者就 开头或结尾 异或下也行（这个开头结尾各异或一下，不等价于不异或mask！！猜测可能是异或后更容易溢出然后不好被卡？）
    x ^= x << 13;
    x ^= x >> 7;
    x ^= x << 17;
    x ^= mask;
...
```cpp
// 做法2：用多项式哈希，记录走过的结点的顺序。偷的zky的，整体代码是重心无根树哈希
#include<bits/stdc++.h>
...
const int N = 55;
const int MO = 1019260817, B = 19491001, INF = 0x3f3f3f3f;

struct Tree { // 无根树两个重心的话
    int A, B;
...
vector<int> e[N];
int n, m, pw[N], hx[N], siz[N], dep[N], rt, rrt, rtm;
vector<PII> ve;

void add(int u, int v) {
    e[u].push_back(v), e[v].push_back(u);
}
void findrt(int u, int f) { // 找重心
    siz[u] = 1;
    int maxn = 0;
    for(auto v: e[u]) {
        if(v == f) continue;
        findrt(v, u);
        siz[u] += siz[v], maxn = max(maxn, siz[v]);
    }
    maxn = max(maxn, n - siz[u]);
    if(maxn < rtm) rtm = maxn, rt = u, rrt = 0;
    else if(maxn == rtm) rrt = u;
}
void dfs(int u, int f) { // 树哈希 nlogn
    hx[u] = 1LL*dep[u] * pw[1] % MO, siz[u] = 1;
    for(auto v: e[u]) {
        if(v == f) continue;
        dep[v] = dep[u] + 1;
        dfs(v, u);
    ...
    for(auto v: e[u]) {
        if(v == f) continue;
        ve.emplace_back(hx[v], siz[v]);
    }
    sort(all(ve)); // 瓶颈，大概可以改成基数排序，变成线性。
    for(auto [hxx, sizz]: ve) {
        hx[u] = (1LL*hx[u] + 1LL*hxx * pw[siz[u]] % MO) % MO, siz[u] += sizz;
    }
}
void work(int x) { // 无根树
    rtm = INF, rrt = 0;
    ...
    findrt(1, -1);
    dep[rt] = 1, dfs(rt, -1), H[x].A = hx[rt];
    if(rrt) dep[rrt] = 1, dfs(rrt, -1), H[x].B = hx[rrt];

    if(H[x].A < H[x].B) swap(H[x].A, H[x].B);
...
int main() {
    ...
    pw[0] = 1;
    for(int i = 1; i <= 50; i ++) pw[i] = 1LL*pw[i-1] * B % MO;
    cin >> m;
...
来自 uoj 博客 - 一种好写且卡不掉的树哈希：

考虑这样一种哈希方式。对于一棵以 $a$ 为根的子树，假设儿子是 1, 2,⋯,$v_1, v_2, \cdots, v_k$，定义子树的哈希 $h(a)=1+\sum_{1\le i\le k} f(h(v_i))$。其中 $h(v_i)$ 是 $v_i$ 对应子树的哈希，$f$ 为一个待定函数。
...
vector<PII> e[N];
int n, st[N], par[N];
LL cfar[N], cdia[N], ccfar[N], ccdia[N], wi[N], d[N];
bool vis[N];
vector<int> cir;
...
bool dfs(int u, int f = -1) {
    st[u] = 1, par[u] = f;
    for(auto [v, w]: e[u]) {
        if(v == f || st[v] == 2) continue;
        if(st[v]) {
            cir.clear();
            for(int x = u; x != v; x = par[x]) cir.pb(x);
            cir.pb(v);
            return true;
        }
...
PLI farest(int u, PII deny = {-1, -1}) {
    vis[u] = true;
    PLI ret = {0, u};
    for(auto [v, w]: e[u]) {
        if(vis[v] || PII(u, v) == deny || PII(v, u) == deny) continue;
        auto t = farest(v, deny);
        t.fi += w;
        if(t.fi > ret.fi) ret = t;
    }
    return ret;
}
LL diam(PII deny = {-1, -1}) {
    memset(vis, 0, sizeof vis);
    auto ret1 = farest(1, deny);
    memset(vis, 0, sizeof vis);
    auto ret2 = farest(ret1.se, deny);
    return ret2.fi;
}
void solve() {
    cin >> n;
    ...
    dfs(1);
    int k = cir.size();
    for(auto x: cir) vis[x] = true;
    for(int i = 0; i < k; i ++) {
        d[i] = farest(cir[i]).fi;
    }
    cir.pb(cir[0]);
    for(int i = 0; i < k; i ++) {
        int u = cir[i], t = cir[i+1];
        for(auto [v, w]: e[u]) {
            if(v == t) {
                wi[i] = w;
                break;
    ...
    for(int i = 1; i < k; i ++) {
        nlen += wi[i-1];
        nfar = max(nfar, d[i-1]) + wi[i-1];
        cfar[i] = max(cfar[i-1], nlen + d[i]);
        cdia[i] = max(cdia[i-1], nfar + d[i]);
    }
    ccfar[k] = ccdia[k] = -INF;
    nlen = 0, nfar = -INF;
    for(int i = k-1; i > 0; i --) {
        nlen += wi[i];
        nfar = max(nfar, d[i+1]) + wi[i];
        ccfar[i] = max(ccfar[i+1], nlen + d[i]);
        ccdia[i] = max(ccdia[i+1], nfar + d[i]);
    }

    PLI ans = {INF, -1};
    // 经过环，断开 i - i+1, [0, i) 2, (i, k) 2, 各一个。 
    for(int i = 0; i < k; i ++) {
        LL cur = max({cfar[i] + ccfar[i+1], cdia[i], ccdia[i+1]});
        if(cur < ans.fi) ans = {cur, i};
    }
    // 不过环，同时保留过环的答案。
    cout << diam({cir[ans.se], cir[ans.se+1]}) << '\n';
    // 也可以 ans 和 不过环的答案取max，具体可以在求环上点的d[]，每个里面都求直径。
    // 这样用树形dp比较方便，维护到叶子最大的距离，下面更新顺序不能反。
    // ans1 = max(ans1, dp[u] + dp[v]+w), dp[u] = max(dp[u], dp[v]+w);
}
```
...
bool circle(int u, int f = -2) {
    st[u] = 1, par[u] = f;
    for(auto [v, id]: e[u]) {
        if((id^1) == f || st[v] == 2) continue;
        if(st[v]) {
            cir.clear();
            cir.push_back(u);
            cir.push_back(v);
        ...
        if(circle(v, id)) return true;
    }
    st[u] = 2;
    return false;
}
void DP(int u, int deny) { // 不能粗糙断边。
    vis[u] = true;
    dp[u][0] = 0, dp[u][1] = w[u];
    for(auto [v, id]: e[u]) {
        if(vis[v] || id == deny || (id^1) == deny) continue;
        DP(v, deny);
        dp[u][1] += dp[v][0];
...
void work(int S) {
    ...
    circle(S);
    LL a1 = 0, a2 = 0;
    int x1 = cir[0], x2 = cir[1];
    DP(x1, cir[2]), a1 = dp[x1][0];
    DP(x2, cir[2]), a2 = dp[x2][0];
    ans += max(a1, a2);
}
int main() {
    ios::sync_with_stdio(false);
    ...
    for(int i = 1; i <= n; i ++) {
        int j; cin >> w[i] >> j;
        e[i].push_back({j, idx++}), e[j].push_back({i, idx++});
    }
    for(int i = 1; i <= n; i ++) {
        if(!V[i]) work(i);
    }
...
-   有源汇可行流
    ...
    连一条 $T \rightarrow S$ 容量为 $+ \infin$ 的边，转化为**无源汇可行流**。再按照**无源汇**的方式建出超级源/汇点， SS 和 TT 。

    当前流量为 $T \rightarrow S$ 的反向边 $S \rightarrow T$的 流量。
...
-   有源汇最大流
    ...
    再二次调整，在 G' 的 SS 、 TT 和 $T \rightarrow S$ 流量为 $+ \infin$ 撤去，这样就得到在 原图 G 上的一组**有源汇可行流**。
...
-   有源汇最小流
    ...
    根据 $S \rightarrow T$ 的最小流等于 $T \rightarrow S$ 最大流的相反数，**有源汇可行流**减去调整后**残留网络的最大流**，即 $f_1 - f_2$ 。
...
-   有源汇费用流
    ...
    初始费用为 $\sum b(u, v)w(u, v)$ ，求**可行流**时，加上 $SS \rightarrow TT$ 的最小费用最大流的费用，二次调整时，再加上产生的费用。

##### 最小割点

求 S 和 T 不连通的最少代价。

点边互化，点拆成**入点**和**出点**，原图的边 $u_{out} \rightarrow v_{in}$ ，因为删点不删边，边权 $+ \infin$ 。
...
-   $c_{u,v}$ 出现负值。
    ...
    此时将**所有边权**（包括所有的 $a_i, b_i$）取反。把求**代价**最大转化为求**贡献**最小。
...
将所有权值取相反数，转化成**最小费用最大流**，遇到有负环的需要用上个解法。
...
-   **最大流**是可以反悔的，所以在加边的时候，不必要把边都清空，可以在跑出来的基础上，加上新跑出来的结果；

    但**费用流**是基于最短路，所以需要重新跑一遍。
...
struct BoundedFlow {
    struct Edge {
        ...
        Edge (int u_, int v_, U lo_, U hi_, U w_): u(u_), v(v_), low(lo_), high(hi_), w(w_) {}
    };
    U flow, cost, w[NN];
    vector<Edge> edges;
...
##### 费用流 Dinic&Dijkstra - $O(m^2logUlogm)$

U 为边的最大容量，基于 Capacity Scaling 。

##### 费用流 Dinic&SPFA - $O(nm^2logU)$
...
```cpp
vector<int> e[N];
int n1, n2, m, mchx[N1], mchy[N2]; // match 的点不能有 0，所以从 1 开始。 pos(x, y) x*m + y + 1
bool vis[N2];
...
for(int i = 1; i <= m; i ++) {
    int x = a[i], y = b[i];
    if(d[x] < d[y] || (d[x] == d[y] && x < y)) e[x].push_back(y);
    else e[y].push_back(x);
}
// 标记，统计
int cnt3 = 0; // m * sqrt(m) 数量级, LL?
for(int i = 1; i <= n; i ++) {
...
```cpp
// version-1 模拟 Boruvka 的过程比较慢 nlog^2n
#include <bits/stdc++.h>
...
const int N = 2e5+10, B = 30, M = N*B, INF = (1<<30);

int idx, tr[M][2], id[M], cnt[M];
int n, a[N], fa[N], min_v[N], min_p[N];
vector<int> ve[N];
...
int main() {
    ...
    sort(a+1, a+n+1); 
    n = unique(a+1, a+n+1) - (a+1); // 相等直接相连，代价为 0。（看题目有时可能不能加）
    for(int i = 1; i <= n; i ++) {
        insert(i);
    ...
    while(tot > 1) {
        for(int i = 1; i <= n; i ++) {
            if(!ve[i].empty()) {
                ...
                for(auto j: ve[i]) insert(j);
            }
        }
        for(int i = 1; i <= n; i ++) {
            if(!ve[i].empty()) {
                int fx = i, fy = leader(min_p[i]), w = min_v[i]; // 等价于 fx = leader(i)
                if(fx != fy) {
                    fa[fy] = fx;
                    if(ve[fy].size() > ve[fx].size()) ve[fx].swap(ve[fy]);
                    // for(auto x: ve[fy]) ve[fx].push_back(x);  // 1294ms
                    ve[fx].insert(ve[fx].end(), all(ve[fy])); // 1216ms
                    ve[fy].clear();
...
```cpp
// version-2 分治，考虑合并的最小代价，较快 nlogn
#include <bits/stdc++.h>
...
const int N = 2e5+10, B = 30, M = N*B, INF = 1<<B;
...
int query(int p, int x, int dep) {
    ...
    for(int k = dep-1; k >= 0; k --) {
        int t = x>>k & 1;
        if(tr[p][t]) p = tr[p][t];
        else p = tr[p][t^1], ret |= 1<<k;
    }
    return ret;
}
LL solve(int p, int l, int r, int dep) {
    if(dep == 0) return 0;
    if(!tr[p][0]) return solve(tr[p][1], l, r, dep-1);
    if(!tr[p][1]) return solve(tr[p][0], l, r, dep-1);
    LL ret = 1<<(dep-1);
    int mn = INF, mid = l;
    while(mid+1 <= r && (a[mid+1]>>(dep-1) & 1) == 0) mid++;
    for(int i = l; i <= mid; i ++) {
        mn = min(mn, query(tr[p][1], a[i], dep-1));
    }
    ret += mn;
    ret += solve(tr[p][0], l, mid, dep-1);
    ret += solve(tr[p][1], mid+1, r, dep-1);
    return ret;
}
int main() {
    ios::sync_with_stdio(0); cin.tie(0);
...
标记永久化。二维平面，插入线段 $log^2V$ ，查询 $x  = x_0$ ，最大的 y 是多少。（V 为横坐标范围）
...
```cpp
// 源代码，信息要满足可减性
// 按注释修改，可以维护不满足可减性的信息。（比如字符串哈希，建议直接写成这样吧）
// 代码中是维护递增的单调栈，如果做到的题是递减的，可以把读入序列倒过来。
#include <bits/stdc++.h>
...
struct Info {
    int l, r;
    db mx;
    int len; // 此时的len，指在当前区间条件下，右区间的值（左边最大值为low的情况下，右区间的结果）
};
struct SegmentTree {
    Info tr[N<<2];
    #define ls(u) ((u)<<1)
    #define rs(u) ((u)<<1|1)
    int calc(int u, db low) {
        auto [l, r, mx, len] = tr[u];
        if(l == r) return mx > low;
        if(tr[ls(u)].mx < low) return 0 + calc(rs(u), low);
        else return calc(ls(u), low) + len-tr[ls(u)].len; // return calc(ls(u), low) + len;
    }
    void pushup(int u) {
        Info &p = tr[u], &ls = tr[ls(u)], rs = tr[rs(u)];
        p.mx = max(ls.mx, rs.mx);
        p.len = ls.len + calc(rs(u), ls.mx); // p.len = calc(rs(u), ls.mx);
    }
    void build(int u, int l, int r) {
        tr[u] = {l, r};
        if(l == r) return;
    ...
    void modify(int u, int x, int c) {
        ...
        int mid = l+r >> 1;
        if(x <= mid) modify(ls(u), x, c);
        else modify(rs(u), x, c);
        pushup(u);
    }
    int query() {
        return tr[1].len; // calc(1, 0);
    }
    #undef ls 
    #undef rs
} sgt;
...
prefix[i] 的 border-1 => prefix[i-1] 的 border
...
求 next 数组，next[i] 向 i 连边，构成 n+1 个节点的树。

性质：

1.   每个前缀的所有 border 是节点 i 到根的链。

2.   有哪些前缀有长度为 x 的 border ： x 的子树。

3.   求两个前缀的公共最大 border ：求LCA。 

### Z 算法（扩展 KMP）

另一种形式的 border， 同时是S的前缀，T的后缀。
...
int query(int l, int r, int x) {
    ...
    for(int k = 24; k >= 0; k --) {
        int t = x>>k & 1;
        if(tr[p][t^1] && (tr[q][t^1] == 0 || tr[q][t^1] && cnt[tr[p][t^1]]-cnt[tr[q][t^1]] > 0)) {
...
**离线**数据结构，复杂度线性（Trie 字符串总长度×字符集大小 + fail O(n)），势能总量为 Trie 节点数总数。

建 Trie 树，BFS 遍历，到一个点，看它父节点的 fail 链，依次看 fail 链的点指向的子节点，有没有等于当前节点的值，有就指向那个等于当前节点值的点。（原始不带优化的AC自动机版本）

写的时候注意，**到底是跳 fail 树还是跳 Trie 图。**

计数看情况，大部分就建 fail 树上拓扑排序求就行 O(mn) ，有时 DFS 序 + 树状数组 在 fail 树上动态差分，利用在点到根节点上的点一定还有没有被回溯，当前点 +1，当前点被回溯后 -1 维护点到根的终止节点的个数 O(nlogn + mlogn) 如 [string ](https://ac.nowcoder.com/acm/problem/14612)。

计数是记所有到达的点，所在 fail 链的总和。

```cpp
// LuoguP5357 加了 fail 优化和 Trie 图优化
#include <bits/stdc++.h>

using namespace std;

const int N = 2e5+10, B = 26;

int idx, tr[N][B], fail[N];
int n, rid[N], ans[N], in[N];
...
void insert(int i, string s) {
    ...
    rid[i] = p; // 必须是单射，从 i 映射到 p，保证 fail 树上的 ans 传递。
}
void build() {
    queue<int> q;
    for(int z = 0; z < B; z ++) {
        if(tr[0][z]) q.push(tr[0][z]);
    }
    while(!q.empty()) {
        ...
        for(int z = 0; z < B; z ++) {
            int v = tr[u][z];
            if(v) {
                fail[v] = tr[fail[u]][z]; // fail 优化
                in[fail[v]]++; // 建 fail 树，v->fail[v]统计
                q.push(v);
            }
            else tr[u][z] = tr[fail[u]][z]; // Trie 图优化
        }
    }
}
void query(string s) {
    ...
    for(auto x: s) {
        int t = x-'a';
        p = tr[p][t]; // 由于是 Trie 图，可以直接这样写
        ans[p]++; // 经过的点打标记，需要在topo上统计出来
    }
}
void topo_sort() {
    queue<int> q;
...
const int N = 5e5+10, B = 26;

int tot, tr[N][B], fail[N], len[N], last;
int cnt[N]; // 按需求的数组...
string s;

void init() {
    tot = 1; // 总点数（0和1被用了），存的“字符串”的长度
    s[0] = -1; // 内部存的“字符串”，方便边界判断
    fail[0] = 1, fail[1] = 0; // fail 互相指
    len[0] = 0, len[1] = -1; // 0偶根，1奇根
    last = 0; // 前一个前缀回文串的位置
}
...
int main() {
    ...
    cin >> s;
    int n = s.size();
    s = "$"+s; // s[0, 1...i-1]，加上第一个额外加的总长度，传进去。
    for(int i = 1; i <= n; i ++) {
        int ch = (s[i]-'a'+cnt[last]) % 26;
        s[i] = (char)(ch + 'a');
        int cur = get_fail(last, i+1);
        if(!tr[cur][ch]) {
            fail[++tot] = tr[get_fail(fail[cur], i+1)][ch];
            len[tot] = len[cur] + 2;
            tr[cur][ch] = tot;

            cnt[tot] = cnt[fail[tot]] + 1; // 自己想要维护的东西
        }
        last = tr[cur][ch];
...
const int N = 3e5+10, B = 26;

int s[N], now = 1;
int tot, tr[N][B], fail[N], len[N], last;
int cnt[N]; // 纯板子没有的，回文串计数。
int n; string str;

void init() {
    tot = now = 1; // 总点数（0和1被用了），存的“字符串”的长度
    s[0] = -1; // 内部存的“字符串”，方便边界判断
    fail[0] = 1, fail[1] = 0; // fail 互相指
    len[0] = 0, len[1] = -1; // 0偶根，1奇根
    last = 0; // 前一个前缀回文串的位置
...
int get_fail(int x) { // ch前回文后缀ch 是不是回文
    while(s[now-len[x]-2] != s[now-1]) x = fail[x];
    return x;
}
void insert(int ch) {
    ...
    if(!tr[cur][ch]) { // 新节点
        int t = new_node(len[cur]+2);
        fail[t] = tr[get_fail(fail[cur])][ch]; // 找新节点的 fail
        tr[cur][ch] = t;
    }
    last = tr[cur][ch];
    cnt[last]++;
...
int main() {
    init();
    cin >> n >> str;
    for(auto x: str) insert(x-'a');
    
    // fail[i] < i，倒着是 fail 拓扑序。
    for(int i = tot; i >= 2; i --) {
        cnt[fail[i]] += cnt[i];
    }
    cnt[0] = cnt[1] = 0;
...
// sa[i] 排名为i的后缀的位置
// rk[i] 位置在i的后缀的排名
// height[i] LCP(sa[i-1], sa[i]) 排名为 i-1 与 i 的 LCP 的长度
char s[N];
int n, sa[N], rk[N], oldrk[N<<1], id[N], key1[N], cnt[N], height[N], H[N];
...
void build() {
    // m 初始时字符范围。
    int m = 1<<7, p = 0;
    for(int i = 0; i <= m; i ++) cnt[i] = 0; // 多组数据清空
    for(int i = 1; i <= n; i ++) cnt[rk[i] = s[i]]++;
    for(int i = 1; i <= m; i ++) cnt[i] += cnt[i-1];
    for(int i = n; i >= 1; i --) sa[cnt[rk[i]]--] = i;
    for(int w = 1; ; w <<= 1, m = p, p = 0) {
        // id[i] 长度为w，第二关键字的rk等于i的位置
        for(int i = n; i > n-w; i --) id[++p] = i;
        for(int i = 1; i <= n; i ++)
            if(sa[i] > w) id[++p] = sa[i] - w;
        for(int i = 0; i <= m; i ++) cnt[i] = 0; // 多组数据清空，不用memset
        // rk[id[i]] 第二关键字的rk等于i的第一关键字的rk
        for(int i = 1; i <= n; i ++) cnt[key1[i] = rk[id[i]]]++;
        for(int i = 1; i <= m; i ++) cnt[i] += cnt[i-1];
        for(int i = n; i >= 1; i --) sa[cnt[key1[i]]--] = id[i];
        for(int i = 0; i <= n; i ++) oldrk[i] = rk[i]; // 多组数据清空，不用memset
        p = 0;
        for(int i = 1; i <= n; i ++)
    ...
    for(int i = 1, k = 0; i <= n; i ++) {
        ...
        // k是后缀i和排名比它小一的后缀的LCP，有需要可以存
        height[rk[i]] = k;
    }
}
LL count_diff() { // 统计本质不同的子串
    LL ret = 0;
    for(int i = 1; i <= n; i ++) {
...
const int N = 1e6+10, B = 26;

// tr 转移
// l 最长长度, ( l[fa[x]], l[x] ]
// 
int tot, last, tr[N<<1][B], l[N<<1], fa[N<<1]; // 2n 点数
int cntA[N<<1], A[N<<1], cnt[N<<1]; // 拓扑序更新，或者把树显式建出来。
int n; LL ans;
char s[N];
...
void insert(int ch) {
    ...
    l[np] = l[p]+1, last = np;
    cnt[np] = 1;
    while(p && !tr[p][ch]) tr[p][ch] = np, p = fa[p];
    if(!p) fa[np] = 1; // root
    else {
        int q = tr[p][ch];
        if(l[q] == l[p]+1) fa[np] = q;
            ...
            while(tr[p][ch] == q) tr[p][ch] = nq, p = fa[p];
        }
    }
}
void build() {
...
//     memset(cnt, 0, sizeof cnt);
    for(int i = 1; i <= tot; i ++) cntA[l[i]]++;
    for(int i = 1; i <= tot; i ++) cntA[i] += cntA[i-1];
    for(int i = tot; i > 0; i --) A[cntA[l[i]]--] = i;
//     int p = 1;
//     for(int i = 0; s[i]; i ++) {
//         cnt[p = tr[p][s[i]-'a']] = 1; // 或者在insert过程中，新建np的时候赋1
//     }
    for(int i = tot; i > 0; i --) {
        int x = A[i];
        cnt[fa[x]] += cnt[x];
...
int main() {
    ...
    build();
    for(int i = 1; i <= tot; i ++) {
        ans = max(ans, 1LL*cnt[i]*l[i] * (cnt[i]>1));
    }
    cout << ans << '\n';
    return 0;
}
...
-   [0, x] 和 [0, y] 中各取一个数取或的最大值为 `x | y | (highbit(x & y) - 1)` 。
-   $\gcd(a^2+b^2, ab) = 1$.
-   
